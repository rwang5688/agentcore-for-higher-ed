"""Live Student Information System query tool.
...
Shared by the CLI agent and the Streamlit app (imported through
``advisor_agent``), so the tool lives in exactly one place.
"""
...
# (keeps unit tests and `--help`-style imports cheap). Config is read here, at
# first call, via the cached get_config() -- never at import time.
_lambda_client = None
