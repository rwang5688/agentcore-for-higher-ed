"""Shared setup for the Peculiar University admission advisor.

Single source of truth for the agent's configuration, the system prompt, and
the ``create_agent`` factory. Both the CLI script (``kb_advisor.py``) and the
Streamlit app (``streamlit_advisor.py``) import from here, so the agent is
defined in exactly one place. The live-data tool lives in ``query_student_db``.
"""
...
def create_agent() -> Agent:
    ...
    The Knowledge Base store is read-only (no ``writable=True``), so it is
    search-only. Callers that need isolated multi-turn context (e.g. one agent
    per Streamlit session) should call this once and reuse the returned agent.
    """
    config = get_config()
